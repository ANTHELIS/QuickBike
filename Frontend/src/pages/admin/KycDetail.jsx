...
const DocCard = ({ label, url }) => {
  if (!url) return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex flex-col items-center gap-2">
      ...
      <p className="text-slate-500 text-xs font-semibold">{label}: Not uploaded</p>
    </div>
  )
  const isPdf = url.includes('.pdf') || url.includes('/raw/')
  return (
    <a href={url} target="_blank" rel="noreferrer" className="block bg-slate-800 border border-slate-700 rounded-xl overflow-hidden hover:border-orange-500/50 transition-colors group">
      {isPdf ? (
        <div className="flex flex-col items-center gap-2 p-6">
          <i className="fa-solid fa-file-pdf text-red-400 text-3xl" />
          <p className="text-slate-400 text-xs font-semibold group-hover:text-white transition-colors">{label} (PDF)</p>
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center">
            <span className="opacity-0 group-hover:opacity-100 transition-opacity text-white text-xs font-bold bg-black/60 px-3 py-1.5 rounded-full">
              <i className="fa-solid fa-expand mr-1" />View Full
            </span>
          </div>
...
const KycDetail = () => {
  ...
  const handleReview = async () => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_BASE_URL}/api/admin/kyc/${id}/review`, {
        action,
  ...
  if (!kyc) return (
    <div className="flex flex-col items-center justify-center h-full min-h-screen gap-3">
      ...
      <button onClick={() => navigate('/admin/kyc')} className="text-orange-400 text-sm font-bold">← Back to list</button>
    </div>
  )
  ...
  return (
    <div className="p-8 font-['Inter']">
      <button onClick={() => navigate('/admin/kyc')} className="flex items-center gap-2 text-slate-400 hover:text-white text-sm font-semibold mb-6 transition-colors">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs">{captain?.email || '—'}</p>
              </div>
            </div>
            <div className="space-y-2 text-sm">
              {[
                ...
                ['Joined', captain?.createdAt ? new Date(captain.createdAt).toLocaleDateString('en-IN') : '—'],
              ].map(([label, val]) => (
                <div key={label} className="flex justify-between items-center border-b border-slate-800 pb-2 last:border-0">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-2">Current Status</p>
            <span className={`text-sm font-bold px-3 py-1.5 rounded-full inline-flex items-center gap-2 ${
              kyc.status === 'approved' ? 'bg-green-500/10 text-green-400' :
              kyc.status === 'rejected' ? 'bg-red-500/10 text-red-400' :
              kyc.status === 'pending'  ? 'bg-orange-500/10 text-orange-400' : 'bg-slate-700 text-slate-400'
            }`}>
              <i className={`fa-solid ${kyc.status === 'approved' ? 'fa-circle-check' : kyc.status === 'rejected' ? 'fa-circle-xmark' : 'fa-clock'}`} />
          ...
          {isPending && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3">
              <p className="text-white font-bold text-sm mb-1">Review Decision</p>
              <div className="flex gap-2">
                <button onClick={() => setAction('approve')} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all border ${action === 'approve' ? 'bg-green-500 text-white border-green-500' : 'bg-slate-800 text-slate-300 border-slate-700 hover:border-green-500/50'}`}>
                  <i className="fa-solid fa-check mr-1.5" />Approve
                </button>
                <button onClick={() => setAction('reject')} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all border ${action === 'reject' ? 'bg-red-500 text-white border-red-500' : 'bg-slate-800 text-slate-300 border-slate-700 hover:border-red-500/50'}`}>
                  <i className="fa-solid fa-xmark mr-1.5" />Reject
                </button>
              </div>
              {action === 'reject' && (
                <textarea
              ...
              {error && <p className="text-red-400 text-xs font-semibold"><i className="fa-solid fa-circle-exclamation mr-1" />{error}</p>}
              {success && <p className="text-green-400 text-xs font-semibold"><i className="fa-solid fa-circle-check mr-1" />{success}</p>}
              {action && (
                <button onClick={handleReview} disabled={submitting} className={`w-full py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${action === 'approve' ? 'bg-green-500 text-white hover:bg-green-600' : 'bg-red-500 text-white hover:bg-red-600'}`}>
                  {submitting ? <><i className="fa-solid fa-circle-notch fa-spin" /> Processing...</> : `Confirm ${action === 'approve' ? 'Approval' : 'Rejection'}`}
                </button>
              )}
            </div>
          )}
