      ...
      <div className="hidden lg:flex lg:w-5/12 relative overflow-hidden p-6 z-10 perspective-1000 pointer-events-none">
        <div className="relative w-full h-full rounded-[40px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.1)] group pointer-events-auto">
          ...
          <img 
            src={getBanner('login')} 
            alt="Commuter" 
            className="absolute inset-0 w-full h-full object-cover object-[80%_center] scale-105 group-hover:scale-110 transition-transform duration-[10s] ease-out opacity-90 pointer-events-none"
          />
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 bg-white/30 backdrop-blur-3xl border border-white/50 rounded-[32px] p-8 w-[85%] shadow-[0_30px_60px_rgba(0,0,0,0.15)] opacity-0 group-hover:opacity-100 group-hover:translate-y-[calc(-50%-10px)] transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)]">
            <div className="w-14 h-14 rounded-full bg-white/80 flex items-center justify-center shadow-lg border border-white/60 mb-6 mx-auto">
               <i className="fa-solid fa-star text-[#F5820D] text-xl" />
            </div>
            <h3 className="text-gray-900 font-extrabold text-2xl text-center mb-3">Premium Context</h3>
            <p className="text-gray-800 font-medium leading-relaxed text-center text-[15px]">
              Every journey crafted for ultimate comfort, speed, and reliability.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-7/12 flex flex-col relative px-6 md:px-12 xl:px-24 z-20 bg-transparent min-h-[100dvh]">
        ...
        <nav className="hidden lg:flex items-center justify-between py-10 z-20 w-full">
          ...
          <div className="flex items-center gap-6 ml-auto">
            <span className="text-[12px] font-medium text-gray-500">New here?</span>
            <Link to="/signup" className="brand-nav-link text-[12px] font-extrabold transition-colors uppercase tracking-widest px-5 py-2.5 rounded-lg">
              Create Account
            </Link>
          </div>
        </nav>
        ...
        <header className="lg:hidden flex items-center justify-between py-6">
          ...
          <Link to="/signup" className="text-[11px] uppercase tracking-widest font-black text-gray-700 bg-gray-100 px-4 py-2 rounded-lg">
            Sign Up
          </Link>
        </header>

        {/* Form Container */}
        <div className="flex-grow flex flex-col justify-center max-w-[440px] w-full mx-auto lg:mx-0 xl:ml-12 2xl:ml-24 pb-12 lg:pb-0 z-10 pt-10 lg:pt-0">
          
          <div className="animate-[fade-in_0.8s_ease-out]">
            <h1 className="brand-heading text-4xl md:text-[54px] font-black font-['Manrope'] mb-4 tracking-tight leading-[1.1] drop-shadow-sm">Welcome Back</h1>
            <p className="brand-text-muted text-[16px] font-medium mb-12">Enter your credentials to access your account.</p>
          </div>
          ...
          <form onSubmit={submitHandler} className="space-y-6 animate-[fade-in_1s_ease-out]">
            ...
            <div className="group">
              ...
              <div className="brand-surface border-2 border-transparent outline outline-1 outline-gray-200 rounded-2xl overflow-hidden brand-focus transition-all duration-300">
                <input
                  type="password"
                  className="w-full bg-transparent py-4 md:py-5 px-5 text-gray-900 font-black placeholder-gray-300 outline-none text-[18px] tracking-widest"
                  placeholder="••••••••"
                  value={password}
            ...
            <button
              type="submit"
              disabled={loading}
              className="brand-btn w-full relative group overflow-hidden py-5 rounded-2xl text-white font-extrabold text-[15px] tracking-[0.1em] flex items-center justify-center gap-3 transition-all duration-300 mt-6 hover:-translate-y-1"
            >
